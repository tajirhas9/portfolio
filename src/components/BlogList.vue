<template>
    <div class="blog-list-container">
        <div class="blog-categories" v-if="uniqueCategoriesArray.length > minCategories">
            <h2 class="heading">Categories</h2>
            <ul>
                <li v-for="category in uniqueCategoriesArray" :key="category" @click="updateCategory(category)"
                    class="blog-category">
                    {{ category }}
                </li>
            </ul>
        ...
        <div class="blog-list" :key="selectedCategory">
            <div v-for="post in posts.data" :key="post.id" class="blog-item">
                <a :href="getBlogPath(post.id)" class="title">{{ post.data.title }}</a>
                <p class="date">
...
const translatePath = useTranslatedPath((props.lang ?? 'en'));

const getBlogPath = (id) => {
    const [lang, path] = id.split('/')

    return translatePath(`/blog/${path}`, lang)
}
...
const uniqueCategories = new Set(
    posts.data.map((post) => post.data?.category).flat().filter(Boolean)
);
